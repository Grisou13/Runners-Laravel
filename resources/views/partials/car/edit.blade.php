        ...
        <div class="panel panel-default">
          <div class="panel-heading">{{ $mode !== null && $mode === "edit" ? "Edit the car " : "Create a car" }}</div>
            <div class="panel-body">
              <form class="form-horizontal" role="form" method="POST" action="{{ $mode == 'edit'? route('car.update',$car) : route('car.store') }}">
                @if($mode == "edit")
                ...
                <div class="form-group{{ $errors->has('comment') ? ' has-error' : '' }}">
                  ...
                  <div class="col-md-6">
                    <textarea {{ $mode !== null && $mode === "edit" ? 'disabled' : ''}} id="comment" name="comment" class="form-control" required autofocus>{{ old('comment', $car->comment)}}</textarea>
                    ...
                    <select {{ $mode !== null && $mode === "edit" ? 'disabled' : ''}} id="car_type_id" name="type" class="form-control" required autofocus>
                      ...
                      @foreach($car_types as $car_type)
                        @if(old("type") == $car_type->id)
                          <option value="{{ $car_type->id }}" selected</option>
