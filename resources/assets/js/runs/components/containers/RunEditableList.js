...
import SubscriptionList from './../views/EditableSubscriptionList'
class RunList extends React.Component
{
    ...
    static gotoEditView(run){
        window.location.href = window.location.href + "/" + run.id + "/edit"
    }
    render(){
                        ...
                        return (
                            <div key={run.id} id={run.id} className={run.status} style={{marginRight:"34px", marginLeft:"23px"}} >
                                <div className="row run" style={{marginLeft:"15px", paddingRight:"-10px"}}>
                                    <RunDetails title={run.title} nb_passenger={run.nb_passenger} date={d} onClick={()=>this.gotoEditView(run)} />
